...
import { compileJs } from "./js-compiler";
import { compileUx } from "./ux-compiler";
...
export function compile(src: string): void
{
    ...
    //
    const { js_code, import_info } = compileJs(jsx_path);
    const js_path = jsx_path.replace(".jsx", ".js");
    writeFileSync(js_path, js_code, "utf8");

    //
    const ux_code = compileUx(jsx_path, import_info);
